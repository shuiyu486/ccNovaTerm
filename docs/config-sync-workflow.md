...
`config.nu` 中的 `claude-env` 是通用启动 wrapper；个人 API endpoint、token 和模型只放在用户本机 env 脚本里，并通过 `claude-env --env-script <path>` 或 `CLAUDE_ENV_SCRIPT` 指定，不应通过 config-sync 同步到项目模板。`claude-dpv4` 仅为兼容旧命令名保留。
